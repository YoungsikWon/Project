...
public class PartnersController {
	...
	// 페이지@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//	@RequestMapping("/partnersMyWishket/{s_email}")
	@RequestMapping("/partnersMyWishket")
	// public String partnersMyWishket(Model mod, @PathVariable String s_email,
	// @RequestParam int pno, HttpSession session, HttpServletRequest req) {
//	public String partnersMyWishket(Model mod, @PathVariable String s_email ,HttpSession session, HttpServletRequest req) {
	public String partnersMyWishket(Model mod, @RequestParam Map<String, Object> pMap, HttpSession session,
			HttpServletRequest req) {
		logger.info("partnersMyWishket호출성공:" + pMap.get("s_email"));
		String s_email = pMap.get("s_email").toString(); // MyWidsket페이지에서 넘긴 pMap(s_email= "")
		int pno = partnersLogic.getUserP_no(s_email);
...
//	public String partnersList(Model mod, @PathVariable String id,@RequestParam int pno , HttpSession session, HttpServletRequest req) {
	...
	public String partnersInterestProject(Model mod, @RequestParam Map<String, Object> pMap, HttpSession session,
			HttpServletRequest req) {
		logger.info("partnersInterestProject호출성공");

		String s_email = pMap.get("s_email").toString();
		int pno = partnersLogic.getUserP_no(s_email);
		Map<String, Object> userInfo = (Map<String, Object>) session.getAttribute(getCookie(req));
